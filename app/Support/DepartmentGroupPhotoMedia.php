...
class DepartmentGroupPhotoMedia
{
    public const DIRECTORY = 'department-group-photos';
    ...
    public static function publicUrlForStoragePath(string $storagePath): string
    {
        ...
        return route('media.department-group-photo', ['filename' => $filename]);
    }
    ...
    private static function isManagedStoragePath(string $path): bool
    {
        return str_starts_with($path, self::DIRECTORY.'/')
            && self::isSafeRelativePath(Str::after($path, self::DIRECTORY.'/'));
    }
